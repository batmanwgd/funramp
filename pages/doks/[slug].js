...
export default function DokPage({
  source,
  frontMatter,
  prevPost,
  nextPost,
...
export const getStaticProps = async ({ params }) => {
  ...
  const { mdxSource, data } = await getDokBySlug(params.slug);
  const prevDok = getPreviousDokBySlug(params.slug);
  const nextDok = getNextDokBySlug(params.slug);
